...
where Self: Sized, Dest: Write
{
    ...
    /// Add the next event.
    fn add_event(&mut self, event_type: EventType, timestamp_ns: u64) -> Result<(), Error>;
...
/// Helper type for wrapping events.
...
where Self: Sized, Dest: Write
{
    ...
    fn add_event(&mut self, _event_type: EventType, _timestamp_ns: u64)
        -> Result<(), Error>
    {
        // Non-packet events can not be saved in the pcap format, drop them.
        Ok(())
    }
    ...
    fn next(&mut self) -> LoaderItem<impl GenericPacket> {
        ...
        match block {
            Block::EnhancedPacket(epb) => {
                let timestamp_ns: u128 = match self.ts_start {
                    Some(start) => (epb.timestamp - start).as_nanos(),
                    None => {
            ...
            Block::Custom(block) if block.pen == GSG_PEN => {
                if let Some(event @ UsbEventBlock { interface_id: 0, .. }) =
                    UsbEventBlock::from_bytes(&block.state, &block.data)
                {
                    let timestamp_ns: u128 = if let Some(ts_start) = self.ts_start
                    {
                        (event.timestamp - ts_start).as_nanos()
                    } else {
                        self.ts_start = Some(event.timestamp);
                        0
                    };
...
where Self: Sized, Dest: Write
{
    ...
    fn add_event(&mut self, event_type: EventType, timestamp_ns: u64)
        -> Result<(), Error>
    {
        let mut event_bytes = Vec::with_capacity(16);
        let event_block = UsbEventBlock {
            interface_id: 0,
            timestamp: Duration::from_nanos(timestamp_ns),
            event_type,
        };
        let state = self.pcap.state();
        event_block.write(state, &mut event_bytes)?;
        self.pcap.write_block(&Block::Custom(
            CustomBlock::new(false, GSG_PEN, state, &event_bytes)
        ))?;
        Ok(())
    }
...
// Private Enterprise Number assigned to Great Scott Gadgets by IANA:
// https://www.iana.org/assignments/enterprise-numbers/?q=62392
const GSG_PEN: u32 = 62392;

// Magic to appear in the high bytes of the first word of our custom block.
const FORMAT_ID: u16 = 0x3343;
const FORMAT_VERSION: u8 = 0x00;

pub struct UsbEventBlock {
...
impl UsbEventBlock {
    fn encode<W: Write, B: ByteOrder>(&self, state: &PcapNgState, writer: &mut W)
        -> Result<(), Error>
    {
    ...
    fn decode<B: ByteOrder>(state: &PcapNgState, mut slice: &[u8])
        -> Option<Self>
    {
        let word = slice.read_u32::<B>().ok()?;

        if (word >> 8) != (FORMAT_ID as u32) << 8 | FORMAT_VERSION as u32 {
            return None
        }

        let interface_id = slice.read_u32::<B>().ok()?;

        let timestamp =
            state.decode_timestamp::<B>(interface_id, &mut slice).ok()?;

        let event_type = EventType::from_code(word as u8)?;
    ...
    fn write<W: Write>(&self, state: &PcapNgState, writer: &mut W)
        -> Result<(), Error>
    {
        match state.section().endianness {
            Endianness::Big => self.encode::<W, BigEndian>(state, writer),
            Endianness::Little => self.encode::<W, LittleEndian>(state, writer),
        }
    }

    fn from_bytes(state: &PcapNgState, slice: &[u8]) -> Option<Self> {
        match state.section().endianness {
            Endianness::Big => Self::decode::<BigEndian>(state, slice),
            Endianness::Little => Self::decode::<LittleEndian>(state, slice),
        }
    }
}
