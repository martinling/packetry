...
use crate::item::{ItemSource, CompletionStatus, SearchResult};
...
impl<Item> ItemNodeSet<Item> where Item: Clone {
    ...
    fn iter_items(&self) -> impl Iterator<Item=(u64, Item)> + '_ {
        self.items
            .iter()
            .map(|(index, node_rc)| (*index, node_rc.borrow().item.clone()))
    }
}
      ...
      for<'r, 's> CaptureSnapshot<'r, 's>: ItemSource<Item, ViewMode>,
{
        ...
        -> Result<u64, Error>
    {
        self.regions
            ...
            .context("No region found for expanded child node")
    }

    fn find_top_level_item(
        &self,
        cap: &mut CaptureSnapshot,
        node_rc: &ItemNodeRc<Item>
    ) -> Result<u64, Error> {
        let index = node_rc.borrow().item_index;
        for (start, region) in self.regions.borrow().iter() {
            match &region.source {
                    ...
                    range.end > index  =>
                {
                    ...
                    let position =
                        ...
                        self.count_within(cap, expanded, &count_range)?;
    ...
    fn expand(
        ...
        let mut update = match (interleaved, &parent.source) {
            ...
            (false, _) => {
                self.split_parent(
                    cap, parent_start, &parent, node_ref, more_after,
                    vec![Region {
            ...
            (true, TopLevelItems()) => {
                ...
                let added = self.count_within(cap, &expanded, &range)?;
                if parent.length == 1 &&
            ...
            (..) => bail!("Unable to expand from {parent:?}")
        };
        ...
        if interleaved {
            while let Some((start, region)) = following_regions.next() {
                ...
                if !self.overlap_region(
                    cap, &mut update, start, &region, node_ref, more_after)?
                {
            ...
            InterleavedSearch(..) => {
                ...
                for (start, region) in following_regions.by_ref() {
                    ...
                    if !self.unoverlap_region(
                        cap, &mut update, start, &region, node_ref)?
                    {
                        // No further overlapping regions.
                        break;
                    }
        ...
        Ok(match &region.source {
            ...
            InterleavedSearch(expanded, range) => {
                ...
                let split_offset = self.count_items(&range_1)? +
                    self.count_within(cap, expanded, &range_1)?;
    ...
    fn count_items(&self, range: &Range<u64>) -> Result<u64, Error> {
    ...
    fn count_in_range(
        ...
        let node = node_ref.borrow();
        cap.count_within(node.item_index, &node.item, range)
    }

    fn count_around_offset(
        &self,
        cap: &mut CaptureSnapshot,
        expanded: &ItemNodeSet<Item>,
        range: &Range<u64>,
        ...
        let length =
            self.count_items(range)? +
            self.count_within(cap, expanded, range)?;
        let rows_before_offset = if offset == 0 {
            0
        } else {
            self.count_to_item(cap, expanded, range, offset - 1, node_ref)?
        };
        let rows_before_end = if end == length {
            self.count_in_range(cap, range, node_ref)?
        } else {
            self.count_to_item(cap, expanded, range, end - 1, node_ref)?
        };
    ...
    fn update_node<T>(&self,
                   ...
                   model: &Model,
                   cap: &mut CaptureSnapshot)
        -> Result<(u64, Option<InterleavedUpdate<Item>>), Error>
                ...
                if let Some(child_rc) = child_weak.upgrade() {
                    ...
                    position = match state {
                        Root(..) =>
                            ...
                            self.find_top_level_item(cap, &child_rc)?,
                        Interleaved(..) => {
        ...
        if let Root(interleaved_updates) = &state {
            ...
            if let Some((child_end, child_rc)) = completed.next() {
                ...
                let (mut expanded, range, old_length) = self.regions
                    ...
                    .context("No interleaved region found")?;
                ...
                for (child_end, child_rc) in completed {
                    ...
                    let new_second = self.count_within(
                        cap, &expanded, &range)?;
            ...
            if top_level_pending > 0 || second_level_pending > 0 {
                ...
                let (new_length, new_top, new_second) = match &source {
                    InterleavedSearch(expanded, full_range) => {
                        ...
                        let full_second =
                            self.count_within(cap, expanded, full_range)?;
                        let full_length = full_top + full_second;
                        let new_length = full_length - old_length;
                        let new_top = full_top - old_top;
                        let new_second = new_length - new_top;
    ...
    fn fetch(&self, position: u64) -> Result<ItemNodeRc<Item>, Error> {
            ...
            match region.source
        {
            TopLevelItems() => (
                self.root.clone(),
                row_index,
                cap.item(None, self.view_mode, row_index)?),
            ChildrenOf(node_ref) => (
                node_ref.clone(),
                row_index,
                cap.item(
                    Some(&node_ref.borrow().item),
                    self.view_mode,
                    row_index)?),
            InterleavedSearch(expanded, range) => {
    ...
    pub fn item(&self, position: u32) -> Option<Object> {
        ...
        let node_or_err_msg = if position >= row_count {
            Err(format!("No row at position {}, only {} rows in model",
                        position, row_count))
        } else {
