...
pub struct EndpointWriter {
    ...
    pub progress_index: IndexWriter<TrafficItemIdOffset, EndpointTransactionId>,
...
pub struct EndpointReader {
    ...
    pub progress_index: IndexReader<TrafficItemIdOffset, EndpointTransactionId>,
    ...
    -> Result<(EndpointWriter, EndpointReader), CaptureError>
{
    ...
    let (progress_writer, progress_reader) = index_stream()?;
    let (end_writer, end_reader) = index_stream()?;
...
impl CompletedTransactions {
    fn next(&mut self, capture: &mut CaptureReader) -> Option<Transaction> {
                ...
                offsets.push(
                    if std::ptr::eq(transfer, longest_transfer) {
                        ...
                        ep_traf.transaction_ids.bisect_range_left(
                            search_range, &pivot_transaction_id)?.value
                    }
                );
            }
